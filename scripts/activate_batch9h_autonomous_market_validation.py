...
def _require_private_repo(gh: str) -> None:
    ...
    if result.returncode != 0 or result.stdout.strip().lower() != "true":
        raise SystemExit("Refusing Batch 9H activation: IIOS telemetry repository is unavailable or not private")
...
def _ensure_validation_issue(gh: str) -> int:
    ...
    body = (
        ...
        "Independent benchmark sidecar only. Recommendations are advisory. No broker or live execution authority."
    )
    created = _capture(
        [
            gh,
...
def _prepare_worktree(git: str) -> tuple[str, str]:
    ...
    if WORKTREE.exists():
        if not (WORKTREE / ".git").exists():
            raise SystemExit(f"9H path exists but is not a git worktree: {WORKTREE}")
        if _capture([git, "status", "--porcelain"], cwd=WORKTREE):
            raise SystemExit("Refusing to replace 9H worktree because it has local changes")
    ...
    if branch_after != branch_before or status_after != status_before:
        raise SystemExit("Refusing activation: live IIOS checkout changed while preparing 9H")
...
def _collector_command(python: Path) -> list[str]:
    return [
        str(python),
        str(WORKTREE / "scripts" / "iios_market_benchmark_collector.py"),
        "--state-dir",
        str(STATE_DIR),
    ]


def _validator_command(python: Path, issue_number: int) -> list[str]:
    return [
        str(python),
        str(WORKTREE / "scripts" / "iios_daily_market_validation.py"),
        "--db",
        str(LEDGER),
        "--state-dir",
        str(STATE_DIR),
...
def _first_benchmark_sample(python: Path) -> dict:
    result = _run(_collector_command(python), cwd=WORKTREE, check=False, capture=True)
    if result.returncode not in (0,):
        raise SystemExit("First Batch 9H independent benchmark sample failed:\n" + (result.stderr or result.stdout or "")[:3000])
    try:
        payload = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise SystemExit(f"Could not parse first Batch 9H collector result: {exc}") from exc
    ...
    if status not in allowed:
        raise SystemExit(f"Unexpected first Batch 9H collector status: {status}")
    if status == "BENCHMARK_SAMPLE_RECORDED":
        if payload.get("independent_of_iios_promotion_decisions") is not True:
            raise SystemExit("First Batch 9H sample did not prove independent benchmark provenance")
        if payload.get("ledger_read") is not False or payload.get("ledger_write") is not False:
            raise SystemExit("First Batch 9H benchmark sample violated the no-ledger collector contract")
        if payload.get("live_execution") is not False:
            raise SystemExit("First Batch 9H benchmark sample did not keep live execution false")
    return payload
...
def main() -> int:
    if sys.platform != "darwin":
        raise SystemExit("Batch 9H activation is intentionally macOS-only for this IIOS runtime")
    git = _require_command("git")
    gh = _require_command("gh")
    _require_command("launchctl")
    _run([gh, "auth", "status"])

    print("IIOS BATCH 9H — AUTONOMOUS MARKET BENCHMARK + MISS LEARNING ACTIVATION")
    ...
    print(f"Benchmark collection cadence: {COLLECTOR_INTERVAL_SECONDS} seconds")
    print(f"After-close validation check cadence: {VALIDATOR_INTERVAL_SECONDS} seconds")
    ...
    _install_agent(
        ...
        program_arguments=_validator_command(python, validation_issue),
    ...
    status_after = _capture([git, "status", "--porcelain"], cwd=LIVE)
    if branch_after != branch_before or status_after != status_before:
        raise SystemExit("Live IIOS checkout changed during Batch 9H activation")
    ...
    summary = {
        ...
        "benchmark_source": "BATCH_9H_INDEPENDENT_YAHOO_SCREENER_SIDECAR",
    ...
    print("\nBatch 9H is active. 9G telemetry remains untouched; 9H only adds independent benchmark collection and after-close validation.")
    return 0
