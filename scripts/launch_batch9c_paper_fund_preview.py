...
WORKTREE = Path("/Users/crm/Documents/GitHub/Investment-Intelligence-OS-batch9c-preview")
BRANCH = "feature/batch9c-paper-fund-operations-ui"
...
def resolve_python() -> Path:
    for candidate in VENV_CANDIDATES:
        if candidate.exists() and os.access(candidate, os.X_OK):
            return candidate
    raise SystemExit(
        "No IIOS backend virtualenv Python found; refusing system Python fallback"
    )
...
def main() -> int:
    ...
    print("IIOS BATCH 9C — ISOLATED PAPER FUND UI PREVIEW")
    ...
    print("Existing 5175 / 8002 / 9A / 9B processes: UNTOUCHED")
    ...
    if WORKTREE.exists():
        if not (WORKTREE / ".git").exists():
            raise SystemExit(
                f"Preview path exists but is not a git worktree: {WORKTREE}"
            )
        run(
            "git",
            "reset",
    ...
    if branch_after != branch_before or status_after != status_before:
        raise SystemExit(
            "Refusing preview: live Batch8 branch or tracked working tree changed"
        )
    ...
    try:
        print(f"Starting read-only preview API on {API_URL} ...")
        api_process = subprocess.Popen(
            [
                str(python),
                "-m",
                "uvicorn",
                "batch9c_preview_api:app",
                "--app-dir",
                "scripts",
                "--host",
                "127.0.0.1",
        ...
        print("Read-only preview API: READY")

        vite = frontend / "node_modules" / ".bin" / "vite"
        if not vite.exists():
            raise RuntimeError(f"Vite executable not found: {vite}")

        print(f"Starting Batch 9C browser preview on {PREVIEW_URL} ...")
        ...
        print("\nStopping Batch 9C preview only...")
        return 0
    finally:
        terminate(ui_process)
        terminate(api_process)
        print("Batch 9C preview stopped. Live IIOS lanes were not stopped.")
