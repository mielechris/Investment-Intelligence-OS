...
def main() -> int:
    ...
    print("Replay authority: UI CURSOR ONLY")
    print("Direct ledger access: NONE")
    print("Backend access: READ_ONLY_GET_ONLY")
    ...
    factory = living.get("factory") or {}
    factory_payload = factory.get("payload") or {}
    cases = factory_payload.get("cases") or []
    telemetry = ((living.get("validation") or {}).get("layers") or {}).get("factory_telemetry") or {}
    telemetry_payload = telemetry.get("payload") or {}
    summary = {
        ...
        "replay_authority": "UI_CURSOR_ONLY",
        "governed_case_count": len(cases),
        "current_promotion_lineage_count": len(telemetry_payload.get("recent_promotions") or []),
    ...
    print(
        "\nBatch 9N case theater is live in the isolated preview. Replay changes only the browser cursor; no factory worker, decision, order or capital permission was modified."
    )
    return 0
