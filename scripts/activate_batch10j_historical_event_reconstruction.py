...
WORKTREE = Path(os.getenv("IIOS_10J_WORKTREE", "/Users/crm/Documents/GitHub/Investment-Intelligence-OS-batch10j-historical-event-reconstruction")).expanduser()
...
EVENT_LABEL = "com.iios.historical-event-reconstruction"
EVENT_INTERVAL_SECONDS = 1800
EVENT_PLIST = LAUNCH_DIR / f"{EVENT_LABEL}.plist"
...
def _clean_generated(git: str) -> None:
    ...
    if unexpected:
        raise SystemExit("10J worktree has non-generated local changes; refusing activation:\n" + "\n".join(unexpected[:20]))
...
def _prepare_worktree(git: str) -> tuple[str, str]:
    ...
    if WORKTREE.exists():
        if not (WORKTREE / ".git").exists():
            raise SystemExit(f"10J path exists but is not a git worktree: {WORKTREE}")
    ...
    if _capture([git, "branch", "--show-current"], cwd=LIVE) != branch_before or _capture([git, "status", "--porcelain"], cwd=LIVE) != status_before:
        raise SystemExit("Live IIOS checkout changed while preparing 10J")
    return branch_before, status_before


def _build(npm: str) -> None:
    _run([npm, "ci"], cwd=FRONTEND)
    _run([npm, "exec", "eslint", "--", "src/LiveFactoryBrowser.tsx", "src/HistoricalEventReconstruction.tsx", "src/ChiefIntelligenceOfficeV2.tsx", "src/HistoricalMarketIntelligence.tsx", "src/QualificationWatch.tsx"], cwd=FRONTEND)
    _run([npm, "run", "build"], cwd=FRONTEND)
    if not (DIST / "index.html").exists():
        raise SystemExit("10J frontend build did not produce dist/index.html")
...
def _plist(label: str, program: list[str], cwd: Path, interval: int, out_name: str) -> Path:
    ...
    payload = {
        ...
        "StandardOutPath": str(LOG_DIR / f"{out_name}.out.log"),
        "StandardErrorPath": str(LOG_DIR / f"{out_name}.err.log"),
    }
    ...
    _run(["launchctl", "bootout", domain, str(path)], check=False, capture=True)
    _run(["launchctl", "bootstrap", domain, str(path)])
    _run(["launchctl", "kickstart", "-k", f"{domain}/{label}"])
    return path


def _run_event_bootstrap(python: Path) -> dict:
    _run([
        str(python), str(WORKTREE / "scripts" / "iios_historical_event_reconstruction.py"),
        "--historical-dir", str(HISTORICAL_DIR),
        "--event-dir", str(EVENT_DIR),
        "--symbols-per-cycle", "2",
    ], cwd=WORKTREE)
    path = EVENT_DIR / "latest_historical_event_reconstruction.json"
    if not path.exists():
        raise SystemExit("10J did not produce its persisted event-reconstruction artifact")
    return json.loads(path.read_text(encoding="utf-8"))


def _publish(python: Path) -> tuple[dict, dict]:
    _run([
...
def _install_workers(python: Path) -> None:
    event_program = [
        str(python), str(WORKTREE / "scripts" / "iios_historical_event_reconstruction.py"),
        "--historical-dir", str(HISTORICAL_DIR),
        "--event-dir", str(EVENT_DIR),
    ...
    _plist(EVENT_LABEL, event_program, WORKTREE, EVENT_INTERVAL_SECONDS, "historical-event-reconstruction")
    final_program = [
        str(python), str(WORKTREE / "scripts" / "iios_final_institutional_publisher.py"),
        "--state-dir", str(STATE_DIR),
        "--telemetry-dir", str(TELEMETRY_DIR),
...
def main() -> int:
    if sys.platform != "darwin":
        raise SystemExit("10J activation is macOS-only for this IIOS runtime")
    _configure_base()
    git = base._require_command("git")
    npm = base._require_command("npm")
    base._require_command("launchctl")
    print("IIOS BATCH 10J — HISTORICAL EVENT RECONSTRUCTION ACTIVATION")
    print("10I whole-stack self-audit: PRESERVED + UPDATED WITH 10J FEEDBACK")
    print("10H 24/7 historical price research: PRESERVED")
    print("10G qualification campaign: PRESERVED")
    print("Existing Backend 8002: UNCHANGED")
    print("10J authority: READ-ONLY EVENT RESEARCH / ADVISORY")
    print("Causal claim authority: FALSE")
    ...
    if backend.get("read_only_aggregation") is not True:
        raise SystemExit("Backend 8002 is not read-only")

    event = _run_event_bootstrap(python)
    safety = event.get("safety") if isinstance(event.get("safety"), dict) else {}
    for key in (
        "causal_claim_authority",
        ...
        if safety.get(key) is not False:
            raise SystemExit(f"10J safety contract violated: {key}")
    if safety.get("read_only_research") is not True or safety.get("advisory_only") is not True or safety.get("human_approval_required") is not True:
        raise SystemExit("10J read-only/advisory/human gate missing")

    _install_workers(python)
    browser_event, office = _publish(python)
    health = _restart_preview(python)
    if health.get("backend_access") != "READ_ONLY_GET_ONLY" or health.get("live_execution") is not False:
        raise SystemExit("10J preview safety boundary failed")
    if base._protected_hashes() != protected:
        raise SystemExit("Protected 9G–9J worker changed")
    if {key: base._hash(path) for key, path in PRESERVED_PLISTS.items()} != preserved:
        raise SystemExit("A preserved 9O–9S or 10H worker changed")
    if _capture([git, "branch", "--show-current"], cwd=LIVE) != branch_before or _capture([git, "status", "--porcelain"], cwd=LIVE) != status_before:
        raise SystemExit("Live checkout changed during 10J activation")
    ...
    print(json.dumps({
        "status": "BATCH10J_HISTORICAL_EVENT_RECONSTRUCTION_PREVIEW",
        "preview_url": f"http://{PREVIEW_HOST}:{PREVIEW_PORT}",
        "event_reconstruction_status": browser_event.get("status"),
        "event_worker": EVENT_LABEL,
        "event_cycle_interval_seconds": EVENT_INTERVAL_SECONDS,
        ...
        "10i_top_recommendation_after_10j": top.get("upgrade_id"),
        "10i_top_action_after_10j": top.get("action_class"),
