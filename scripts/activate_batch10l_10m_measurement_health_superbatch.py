...
def build_frontend(npm: str) -> None:
    ...
    if not (DIST / "index.html").exists():
        raise SystemExit("10L-10M frontend build missing dist/index.html")
...
def main() -> int:
    if sys.platform != "darwin":
        raise SystemExit("10L-10M activation is macOS-only")
    configure_base()
    git = base._require_command("git")
    npm = base._require_command("npm")
    base._require_command("launchctl")
    print("IIOS BATCH 10L–10M — MEASUREMENT + DATA HEALTH + COST GOVERNOR")
    print("10L benchmark attribution: MEASUREMENT ONLY")
    print("10M data health watchdog: OBSERVABILITY ONLY")
    print("10M model/tool cost governor: EXACT-COST OBSERVABILITY + ADVISORY BUDGET POLICY")
    print("Cost estimates when provider cost is unknown: PROHIBITED")
    print("Auto model routing/provider changes: FALSE")
    ...
    cost_program = [str(python), str(WORKTREE / "scripts" / "iios_model_cost_governor.py"), "--cost-dir", str(COST_DIR)]
    health_program = [str(python), str(WORKTREE / "scripts" / "iios_data_health_watchdog.py"), "--state-dir", str(STATE_DIR), "--telemetry-dir", str(TELEMETRY_DIR), "--historical-dir", str(HISTORICAL_DIR), "--event-dir", str(EVENT_DIR), "--macro-dir", str(MACRO_DIR), "--benchmark-dir", str(BENCHMARK_DIR), "--cost-dir", str(COST_DIR), "--browser-dir", str(DIST)]
    final_program = [str(python), str(WORKTREE / "scripts" / "iios_measurement_health_publisher.py"), "--state-dir", str(STATE_DIR), "--telemetry-dir", str(TELEMETRY_DIR), "--historical-dir", str(HISTORICAL_DIR), "--event-dir", str(EVENT_DIR), "--macro-dir", str(MACRO_DIR), "--benchmark-dir", str(BENCHMARK_DIR), "--health-dir", str(HEALTH_DIR), "--cost-dir", str(COST_DIR), "--browser-dir", str(DIST)]

    run(benchmark_program, cwd=WORKTREE)
    run(cost_program, cwd=WORKTREE)
    install_plist(BENCHMARK_LABEL, benchmark_program, BENCHMARK_INTERVAL, "benchmark-alpha-attribution")
    install_plist(COST_LABEL, cost_program, COST_INTERVAL, "model-cost-governor")
    ...
    cost = json.loads((DIST / "model_cost_governor.json").read_text(encoding="utf-8"))
    office = json.loads((DIST / "chief_intelligence_office_v2.json").read_text(encoding="utf-8"))
    health_check = restart_preview(python)
    if health_check.get("backend_access") != "READ_ONLY_GET_ONLY" or health_check.get("live_execution") is not False:
        raise SystemExit("10L-10M preview safety boundary failed")
    if base._protected_hashes() != protected:
        raise SystemExit("Protected 9G–9J worker changed")
    if {key: base._hash(path) for key, path in PRESERVED.items()} != preserved:
        raise SystemExit("Preserved 9O–10K worker changed")
    if capture([git, "branch", "--show-current"], cwd=LIVE) != branch_before or capture([git, "status", "--porcelain"], cwd=LIVE) != status_before:
        raise SystemExit("Live checkout changed during 10L-10M")
    ...
    c_safety = cost.get("safety") if isinstance(cost.get("safety"), dict) else {}
    for payload, keys in (
        (b_safety, ("capital_authority", "broker_connection_authority", "trade_execution_permission", "live_execution")),
        (h_safety, ("capital_authority", "trade_execution_permission", "live_execution")),
        (c_safety, ("auto_change_model_routing", "auto_change_provider", "auto_disable_models", "capital_authority", "trade_execution_permission", "live_execution")),
    ):
        for key in keys:
            if payload.get(key) is not False:
                raise SystemExit(f"10L-10M safety violation: {key}")

    opener = shutil.which("open")
    if opener:
        run([opener, f"http://{PREVIEW_HOST}:{PREVIEW_PORT}"], check=False)
    rolling = cost.get("rolling_7d") if isinstance(cost.get("rolling_7d"), dict) else {}
    today = cost.get("today") if isinstance(cost.get("today"), dict) else {}
    print(json.dumps({
        "status": "BATCH10L_10M_MEASUREMENT_HEALTH_COST_PREVIEW",
        ...
        "cost_governor_status": cost.get("status"),
        "cost_budget_state": cost.get("budget_state"),
        "cost_today_exact_spend_usd": today.get("exact_spend_usd"),
        "cost_rolling_7d_exact_spend_usd": rolling.get("exact_spend_usd"),
        "cost_exact_coverage_pct": rolling.get("exact_cost_coverage_pct"),
        "cost_enforcement_hooks_connected": cost.get("enforcement_hooks_connected"),
