...
def _patch_office(office: dict[str, Any], benchmark_payload: dict[str, Any], health_payload: dict[str, Any]) -> dict[str, Any]:
    result = dict(office)
    ...
    layers.extend([
        ...
        {"layer": "10M", "name": "Data Health Watchdog", "status": health_payload.get("status")},
    ])
...
def publish_all(*, state_dir: Path, telemetry_dir: Path, historical_dir: Path, event_dir: Path, macro_dir: Path, benchmark_dir: Path, health_dir: Path, browser_dir: Path) -> dict[str, Any]:
    ...
    office = _patch_office(office, benchmark_payload, health_payload)
    ...
    return {
        **base_result,
        "artifact_count": int(base_result.get("artifact_count") or 0) + 2,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Refresh IIOS browser artifacts through Batch 10L/10M.")
