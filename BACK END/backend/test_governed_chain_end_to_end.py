...
class GovernedChainEndToEndTests(
    unittest.TestCase
):
    ...
    def seed_sizing_inputs(
        self,
        case_id,
    ):
        profile_id = (
            f"paper_sizing_profile_"
            f"{case_id}"
        )
    ...
    def build_good_chain(
        ...
        entry_watch = (
            self.entry.classify_entry_state(
                capital=capital,
                previous={
                    "stage":
                        "WAIT_FOR_ENTRY",
    ...
    def test_01_good_trade_reaches_one_paper_order(self):
        ...
        # Live execution NEVER becomes available.
        self.assertFalse(
            result[
                "trade_execution_permission"
            ]
    ...
    def test_07_stale_governed_state_is_rejected(self):
        ...
        # Quote stays the same, but the governed
        # reward/risk state changes after authorization.
        changed = {
            **chain["capital"],
            "reward_risk": 1.76,
        }
