...
POLICY_SCHEMA_V1 = "iios-unattended-market-session-policy-v1"
POLICY_SCHEMA = "iios-unattended-market-session-policy-v2"
...
DAILY_CEILING = 200
STAGE_A_MAXIMUM = 100
STAGE_A_AUTHORIZED_ALLOWANCE = 50
...
POLICY_FIELDS = frozenset({
    ...
    "authority", "stage_a_authorized_allowance_credits", "cost_contract_hash", "content_hash",
})
...
def request_plan(policy_identity: str) -> tuple[dict[str, Any], ...]:
    windows = (("OPENING_SESSION","MARKET_SNAPSHOT","06:30","07:00"),("POINT_IN_TIME_OHLCV","HISTORICAL_OHLCV","06:30","08:00"),("APPLICABLE_FACTS","COMPANY_FACTS_OR_INSTRUMENT_PROFILE","07:00","10:00"),("INTRADAY_MARK","MARKET_SNAPSHOT","09:30","10:30"),("CLOSING_MARK","MARKET_SNAPSHOT","12:55","13:05"))
    rows=[]
    for room,(instrument,_) in PILOT_BY_PRODUCT.items():
        for category,endpoint,earliest,latest in windows:
            applicable="COMPANY_FACTS" if instrument=="MU" and category=="APPLICABLE_FACTS" else ("INSTRUMENT_PROFILE" if category=="APPLICABLE_FACTS" else endpoint)
            seed="|".join((TARGET_DATE,instrument,applicable,category,earliest,policy_identity,"fd-contract-v1"))
            rows.append({"instrument":instrument,"product_room":room,"endpoint":applicable,"evidence_category":category,"earliest":earliest,"latest":latest,"estimated_cost":1,"request_identity":"stage-a-"+hashlib.sha256(seed.encode()).hexdigest(),"provider_contract":"fd-contract-v1","retry":False})
    return tuple(rows)


def operational_request_plan() -> tuple[dict[str, Any], ...]:
    from .provider_readiness import revised_request_plan
    return revised_request_plan()


def plan_identity(policy_identity: str) -> str:
    del policy_identity
    from .provider_readiness import revised_plan_identity
    return revised_plan_identity()
...
def make_policy(*, owner_identity: str, approval_timestamp: str, command_time: datetime,
                installed_commit: str, authorized_commit: str, cost_binding: dict[str, Any],
    ...
    value = {
        ...
        "approved_commit": authorized_commit, "request_plan_identity": cost_binding.get("request_plan_identity"),
        ...
        "stage_a_maximum": STAGE_A_MAXIMUM, "stage_a_authorized_allowance_credits": STAGE_A_AUTHORIZED_ALLOWANCE,
        "cost_contract_hash": cost_binding.get("cost_contract_hash"), "daily_ceiling": DAILY_CEILING, "automatic_retry": False,
    ...
    return validate_policy(value, installed_commit=installed_commit)


def validate_policy(value: Any, *, installed_commit: str | None = None) -> dict[str, Any]:
    ...
    if value.get("controller_schema") != STATE_SCHEMA_V2 or not _commit(value.get("approved_commit")):
        raise ValueError("POLICY_BINDING_INVALID")
    if installed_commit is not None and (not _commit(installed_commit) or value["approved_commit"] != installed_commit):
        raise ValueError("POLICY_BINDING_INVALID")
    ...
    if (value.get("stage_a_maximum") != 100 or value.get("stage_a_authorized_allowance_credits") != 50
            or value.get("daily_ceiling") != 200):
        raise ValueError("POLICY_BUDGET_INVALID")
    if not isinstance(value.get("cost_contract_hash"), str) or len(value["cost_contract_hash"]) != 64:
    ...
    from .provider_readiness import revised_plan_identity
    if value.get("request_plan_identity") != revised_plan_identity(): raise ValueError("POLICY_PLAN_INVALID")
...
def validate_session(value: Any, policy: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(ids,list) or len(ids)!=len(set(ids)) or any(x not in {r['request_identity'] for r in operational_request_plan()} for x in ids): raise ValueError("REQUEST_IDENTITY_INVALID")
    ...
    if (value["planned"] != 50 or value["completed"] != len(ids)
            or value["released_credits"] > policy["stage_a_authorized_allowance_credits"]
            or value["confirmed_credits"]+value["ambiguous_credits"] > active_allowance): raise ValueError("ACCOUNTING_INVALID")
...
def preflight(gates: dict[str,bool], *, costs: dict[str,int] | None, emergency_closed: bool=False) -> tuple[bool,str|None]:
    ...
    endpoints={r["endpoint"] for r in operational_request_plan()}
    ...
    projected=sum(costs[r["endpoint"]] for r in operational_request_plan())
...
class OfflineSession:
    ...
    def release_stage_a(self, gates: dict[str,bool], costs: dict[str,int]|None) -> str:
        if self.state["released_credits"] != 0: raise ValueError("STAGE_A_ALLOWANCE_CONTRACT_UNSUPPORTED")
        ...
        projected_cost=sum(costs[row["endpoint"]] for row in operational_request_plan())
        if (self.policy["stage_a_authorized_allowance_credits"] != 50
                or projected_cost > self.policy["stage_a_authorized_allowance_credits"]
                or projected_cost > self.policy["stage_a_maximum"]):
            raise ValueError("STAGE_A_ALLOWANCE_CONTRACT_UNSUPPORTED")
        self.state["released_credits"]=self.policy["stage_a_authorized_allowance_credits"]
        self.state["content_hash"]=_hash(self.state); validate_session(self.state,self.policy); return "STAGE_A_RUNNING"
    def observe(self,row:dict[str,Any], provider:Callable[[dict[str,Any]],str]) -> str:
        ...
        cost=row.get("confirmed_cost",row.get("estimated_cost"))
...
def browser_projection(policy: dict[str,Any]|None,state:dict[str,Any]|None,*,read_at:str) -> dict[str,Any]:
    if policy is None or state is None:
        return {"schema_version":BROWSER_SCHEMA,"policy_installed":False,"policy_status":"NOT_INSTALLED","session_date":TARGET_DATE,
                ...
                "stage_a_status":"LOCKED","policy_schema":POLICY_SCHEMA,"commit_binding":"UNAVAILABLE","stage_a_authorized_allowance":50,"stage_a_maximum":100,"released_credits":0,"planned":50,"completed":0,"failed":0,
    ...
    return {"schema_version":BROWSER_SCHEMA,"policy_installed":True,"policy_status":"ONE_DAY_IMMUTABLE",
            ...
            "policy_schema":policy["schema_version"],"commit_binding":"MATCH","stage_a_authorized_allowance":policy["stage_a_authorized_allowance_credits"],
