...
class PolicyContractTests(unittest.TestCase):
    def policy(self): return make_policy(owner_identity=OWNER,approval_timestamp=APPROVAL,command_time=NOW)
    ...
    def test_policy_is_one_day_immutable_and_bound(self):
        ...
        self.assertEqual((p["session_date"],p["timezone"],p["approved_commit"]),(TARGET_DATE,TARGET_ZONE,APPROVED_COMMIT)); self.assertEqual(p["authority"],LOCKED_AUTHORITY)
    ...
    def test_policy_tamper_and_wrong_binding_rejected(self):
        for field,value,category in (("approved_commit","0"*40,"POLICY_BINDING_INVALID"),("recurring",True,"POLICY_RECURRENCE_INVALID"),("content_hash","0"*64,"POLICY_HASH_INVALID")):
            ...
            with self.subTest(field=field),self.assertRaisesRegex(ValueError,category): validate_policy(p)
    def test_plan_has_exactly_fifty_bound_unique_identities(self):
        ...
        facts=[x for x in rows if x["evidence_category"]=="APPLICABLE_FACTS"]; self.assertEqual(sum(x["endpoint"]=="COMPANY_FACTS" for x in facts),1)
...
class ScheduleAndSafetyTests(unittest.TestCase):
    ...
    def test_cost_gate_and_all_preflight_gates(self):
        endpoints={x["endpoint"] for x in request_plan("tuesday-2026-09-08-stage-a")}; costs={x:1 for x in endpoints}
    ...
    def test_end_to_end_stub_restart_no_double_charge_and_close(self):
        p=self.policy(); s=initial_state(p); s=transition(s,p,"TUESDAY_WAITING_FOR_PREFLIGHT"); s=transition(s,p,"TUESDAY_PREFLIGHT_RUNNING")
        endpoints={x["endpoint"] for x in request_plan(p["policy_identity"])}; session=OfflineSession(p,s); self.assertEqual(session.release_stage_a(gates(),{x:1 for x in endpoints}),"STAGE_A_RUNNING")
        rows=request_plan(p["policy_identity"]); first=rows[:23]; rest=rows[23:]
    ...
    def test_ambiguous_is_charged_failed_is_not_retried_and_emergency_is_idempotent(self):
        p=self.policy(); s=initial_state(p); s=transition(s,p,"TUESDAY_WAITING_FOR_PREFLIGHT"); s=transition(s,p,"TUESDAY_PREFLIGHT_RUNNING")
        endpoints={x["endpoint"] for x in request_plan(p["policy_identity"])}; x=OfflineSession(p,s); x.release_stage_a(gates(),{k:1 for k in endpoints}); rows=request_plan(p["policy_identity"])
    ...
    def test_persistent_tick_fails_closed_at_unknown_cost_without_calls(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            _,state=service._read(store); self.assertEqual((state["failure_category"],state["released_credits"],state["completed"]),("ENDPOINT_COST_UNKNOWN",0,0))
