...
from alpha_session_execution import (digest, safe_root, relative, read_pinned,
    verify_destination, publish, read_record, execute_day)
...
def verify_runtime(admission):
    ...
    try:
        ...
        if m['mode'] == 'LIVE_QUALIFICATION':
            ...
            required = {'provider_gateway_live_contract.py', 'provider_gateway_credentials.py', 'provider_gateway_transport.py', 'provider_gateway_wire.py', 'provider_gateway_qualification.py', 'provider_gateway_contract.py', 'provider_gateway_adapters.py', 'truth_spine_contract.py', 'alpha_session_execution.py', 'alpha_market_baseline.py', 'provider_gateway_https.py'}
...
def qualify(manifest, account, runtime, *, expected, credential_backend, network,
    ...
    return execute_day(m, a, clock=clock, expected_bulk_previous=expected_bulk_previous,
        dispatch=lambda fd, slot, reserved: _qualify(manifest, account, runtime,
            _bulk_permit=_BulkPermit(dict(admission.parents)['manifest'], fd, slot, reserved), **args),
        verify_receipt=verify_qualification_receipt)
