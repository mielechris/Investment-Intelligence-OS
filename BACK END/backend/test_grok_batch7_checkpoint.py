...
class GrokBatch7CheckpointTests(unittest.TestCase):
    def test_checkpoint_marks_forward_cycle_ready_but_value_proof_pending(self):
        out = checkpoint.build_batch7_checkpoint()
        self.assertEqual(out["stage"], "7C_FORWARD_VALUE_PROOF")
        self.assertEqual(out["checkpoint"], "FORWARD_VALUE_CYCLE_READY_AWAITING_TIME_SEPARATED_OBSERVATIONS")
        self.assertEqual(out["resume_phrase"], "pick up Batch 7")
        self.assertEqual(out["resume_from"], "RUN_FIRST_FORWARD_VALUE_CYCLE")
