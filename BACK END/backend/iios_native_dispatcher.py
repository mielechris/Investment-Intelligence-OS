...
class NativeContext:
    def __init__(self,manifest,parent,root):
        ...
        self.deadline=None;self.children=[];self.query_handles=[];self.events=[]
...
def run(manifest,parent):
    ...
    def builtin(row,deadline,budget):
        ...
        if stage==STAGES[0]:admit_source(manifest)
            ...
            terminal_categories(os.environ.get('TERM_PROGRAM'),[os.isatty(fd) for fd in (0,1,2)],any(k in os.environ for k in ('VSCODE_PID','VSCODE_IPC_HOOK_CLI','CODEX_THREAD_ID','CODEX_SANDBOX_NETWORK_DISABLED')),host,manifest['host'])
        elif stage==STAGES[2]:
