...
type BrowserView = "floor" | "expansion" | "control";

function viewFromLocation(): BrowserView {
  return window.location.hash === "#expansion-wing" ? "expansion" : window.location.hash === "#control-room" ? "control" : "floor";
}
...
export default function LiveFactoryBrowser() {
  ...
  const chooseView = useCallback((next: BrowserView, historyMode: "push" | "replace" = "push") => {
    setView(next);
    const fragment = next === "floor" ? "factory-floor" : next === "expansion" ? "expansion-wing" : "control-room";
    window.history[historyMode === "push" ? "pushState" : "replaceState"]({ iiosView: next }, "", `#${fragment}`);
  }, []);

  useEffect(() => {
    const pop = () => setView(viewFromLocation());
    const escape = (event: KeyboardEvent) => {
      if (event.key === "Escape" && view !== "floor") {
        chooseView("floor");
        requestAnimationFrame(() => floorButton.current?.focus());
      }
    };
