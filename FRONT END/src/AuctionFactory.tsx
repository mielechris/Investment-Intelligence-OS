...
export default function AuctionFactory({ model, onOpenRoom }: { model: AuctionModel; onOpenRoom: (id: AuctionRoomId) => void }) {
  ...
  return <div className={`auction-factory auction-light--${model.lighting}`} onPointerMove={shiftPerspective} onPointerLeave={(event) => { event.currentTarget.style.setProperty("--parallax-x", "0"); event.currentTarget.style.setProperty("--parallax-y", "0"); }}>
    ...
    <div className="auction-building" aria-label="Interactive multi-level architectural cutaway of the IIOS factory">
      ...
      <div className={`auction-max-walkway ${model.activeRoom ? "is-watching" : ""}`}><div className="auction-walkway-rail" aria-hidden="true"/><CinematicCharacterPortrait characterKey="max" variant="boss"/><div><small>FOREMAN / OBSERVER</small><b>MAX’S WALKWAY</b><span>{model.quiet ? "Quiet-floor patrol" : `Watching ${model.activeRoom?.toUpperCase()}`}</span></div></div>
...
function Room({ room, index, state, open }: { room: AuctionRoom; index: number; state: string; open: () => void }) {
  ...
  return <button className={`auction-room auction-room--${room.id} auction-room--${state}`} style={{ "--room-index": index } as React.CSSProperties} onClick={open} aria-label={`Open ${room.label}; ${state}`}>
    ...
    <div className="auction-room__set" aria-hidden="true"><span className="auction-room__lamp"/><span className="auction-room__window"/><span className="auction-room__desk"><i/><i/><i/></span></div>
...
export function RoomView({ roomId, model, close }: { roomId: AuctionRoomId; model: AuctionModel; close: () => void }) {
  ...
  return <div ref={dialogRef} className={`auction-room-modal auction-room-modal--${room.id}`} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} tabIndex={-1} onMouseDown={close}>
    <section onMouseDown={(event) => event.stopPropagation()}>
      ...
      <header className="auction-interior-heading"><span>{room.shortLabel} / {model.rooms[room.id].toUpperCase()}</span><h2 id={titleId}>{room.label}</h2><p id={descriptionId}>{room.purpose}</p></header>
      ...
      <div className="auction-interior-console" aria-hidden="true"><i/><i/><i/><i/><span>OBSERVATION BUS / READ ONLY</span></div>
      <dl><dt>Authoritative source</dt><dd>{room.source}</dd><dt>Current state</dt><dd>{model.rooms[room.id].toUpperCase()}</dd><dt>Provenance</dt><dd>{model.generatedAt ?? "UNKNOWN"}</dd><dt>Authority</dt><dd>READ-ONLY · NO LEDGER WRITE · NO TRADE EXECUTION · LIVE EXECUTION FALSE</dd></dl>
    </section>
  </div>;
}
