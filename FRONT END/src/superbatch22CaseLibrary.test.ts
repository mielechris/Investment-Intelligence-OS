...
test("case and product surfaces are primary, bounded, and responsive", () => {
  ...
  assert.match(products, /Twenty-four product rooms participate in Tuesday testing across ten live-evidence pilots, four listed-equity source-readiness rooms, and ten specialized structural test rooms/); assert.doesNotMatch(products, /\$240,000/); assert.match(products, /Operational paper fund", "\$10,000 SEPARATE/); assert.match(products, /Paper-test-ready products", 0/); assert.match(products, /Research-only products", 11/); assert.match(products, /Incomplete products", 13/); assert.match(products, /AUTHENTIC PRODUCT EVIDENCE REQUIRED/);
