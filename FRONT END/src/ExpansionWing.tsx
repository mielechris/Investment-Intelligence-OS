...
const displayValue = (value: unknown) => value == null ? "UNKNOWN / NOT REPORTED" : typeof value === "boolean" ? (value ? "Yes" : "No") : String(value);
function SectionSummary({ value }: { value: Record<string, unknown> | null | undefined }) {
  if (!value) return <p className="wing-empty-summary">No sanitized evidence available.</p>;
  return <><dl className="wing-summary">{Object.entries(value).filter(([,item])=>typeof item !== "object").map(([key,item])=><div key={key}><dt>{humanize(key)}</dt><dd>{displayValue(item)}</dd></div>)}</dl><details className="wing-technical"><summary>Technical details</summary><pre>{JSON.stringify(value,null,2)}</pre></details></>;
}
function statusLabel(key: string, state: string, data: unknown) {
  if (key !== "projection_activation" || !data || typeof data !== "object") return state;
  return (data as {reader_state?:string}).reader_state === "READY" ? "READER ACTIVE" : `READER ${(data as {reader_state?:string}).reader_state??"UNAVAILABLE"}`;
}
...
export default function ExpansionWing() {
  ...
  return <section className="expansion-wing" aria-labelledby="expansion-wing-title" data-mode={FIXTURE_MODE ? "fixture" : "read-only-live"}>
    ...
    <section className="wing-lanes" aria-label="Multi-asset research lanes">
      ...
      <div>{MULTI_ASSET_LANES.map((lane)=>{const detail=multiData?.lane_states?.[lane];const state=typeof detail==="string"?detail:detail?.state??"UNAVAILABLE";const missing=typeof detail==="object"?detail.missing_evidence:undefined;return <article key={lane}><span>{lane.replaceAll("_"," ")}</span><strong className={`wing-state wing-state--${state.toLowerCase()}`}>{state}</strong>{typeof detail==="object"?<small>{detail.instrument_basis??"UNKNOWN BASIS"} · {detail.freshness??"UNKNOWN"}<br/>Research {detail.research_eligible?"eligible":"blocked"} · Paper {detail.paper_eligible?"eligible":"blocked"}{missing?<><br/><span className="wing-reason-summary">{humanize(missing)}</span><details className="wing-machine-detail"><summary>Failure category</summary><code>{missing}</code></details></>:null}</small>:null}</article>;})}</div>
    </section>
    ...
    <section className="wing-observatory" aria-label="Professional strategy and paper research status">{["professional_strategy_observatory","method_manager_scoreboard","paper_research_sleeves"].map((key)=>{const section=status?.sections[key];return <article key={key}><h3>{labels[key]}</h3><strong className={`wing-state wing-state--${(section?.state??"UNAVAILABLE").toLowerCase()}`}>{section?.state??"UNAVAILABLE"}</strong><SectionSummary value={section?.data as Record<string,unknown>|null|undefined}/></article>;})}</section>
    <div className="wing-truth-grid" aria-label="Expansion Wing truth states">{Object.entries(labels).map(([key,label]) => { const section=status?.sections[key];const state=section?.state??"UNAVAILABLE";const visible=statusLabel(key,state,section?.data); return <article key={key}><span>{label}</span><strong title={state} aria-label={`${label}: ${visible}. Contract state: ${state}`} className={`wing-state wing-state--${state.toLowerCase()}`}>{visible}</strong></article>; })}</div>
