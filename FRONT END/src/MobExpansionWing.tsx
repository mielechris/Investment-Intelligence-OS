...
function TuesdayCommandCenter({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  return <div className="mew-grid mew-tuesday-grid" data-controller-version={controllerV2 ? "v2" : "v1"}>
    <Panel id="tuesday-command" eyebrow="OPENING-DAY CONTROL" title="Tuesday Test Day Command Center" state="NOT_ACTIVATED" className="mew-wide">
      ...
      <section className={`mew-controller-version is-${controllerV2 ? "future-v2" : "authentic-v1"}`} aria-label="Authenticated controller version">
        <header><span>{controllerV2 ? "SYNTHETIC_FIXTURE_NON_LIVE" : "AUTHENTIC CURRENT OPERATIONAL STATE"}</span><strong>{controllerV2 ? "FUTURE REVIEW — NOT OPERATIONAL" : "INSTALLED DISABLED CONTROLLER"}</strong></header>
        <dl>
