...
function TuesdayCommandCenter({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  return <div className="mew-grid mew-tuesday-grid">
    <Panel id="tuesday-command" eyebrow="OPENING-DAY CONTROL" title="Tuesday Test Day Command Center" state="NOT_ACTIVATED" className="mew-wide">
      <p className="mew-fixture-banner">PAPER RESEARCH / NO LIVE CAPITAL</p>
...
function ProductAccountDirectory({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  const selectedRow = PRODUCT_DESKS.find(([, name]) => name === selected) ?? PRODUCT_DESKS[0];
  return <Panel id="product-account-directory" eyebrow="24 SEPARATE RESEARCH ACCOUNTS" title="Product Account Directory" state="AVAILABLE_EMPTY" className="mew-wide">
    <p>Each product has an independent synthetic $10,000 research basis. The accounts are never aggregated and do not alter the separate operational paper fund.</p>
    ...
    <div className="mew-account-truth"><strong>24 independent $10,000 comparison bases</strong><span>Operational paper remains NAV {scalar(books.nav)} · cash {scalar(books.cash)} · positions {scalar(books.positions)} · orders {scalar(books.orders)} · fills {scalar(books.fills)}</span></div>
    <div className="mew-account-directory" aria-label="Twenty-four directly visible product accounts">{PRODUCT_DESKS.map(([department, name, classification], index) => { const researchOnly=index<11; return <button type="button" key={name} aria-pressed={selected === name} onClick={() => setSelected(name)}><b>{String(index + 1).padStart(2, "0")} · {name}</b><span>Synthetic NAV $10,000 · cash $10,000</span><small>{department} · {classification}</small><em>Positions 0 · trades 0</em><em>Evidence {researchOnly ? "RESEARCH ONLY" : "INCOMPLETE"} · source NOT ACTIVATED</em><em>Eligibility FALSE · blocker AUTHENTIC PRODUCT EVIDENCE REQUIRED</em></button>; })}</div>
    <section className="mew-account-detail" aria-live="polite"><header><div><span>SELECTED PRODUCT ACCOUNT</span><h3>{selectedRow[1]}</h3></div><Status state="UNAVAILABLE" /></header><MetricGrid values={[["Department", selectedRow[0]], ["Exposure class", selectedRow[2]], ["Starting basis", "$10,000"], ["Synthetic NAV", "$10,000"], ["Synthetic cash", "$10,000"], ["Positions", 0], ["Trades", 0], ["Evidence", "UNAVAILABLE"], ["Source", "NOT ACTIVATED"], ["Research eligible", false], ["Paper eligible", false], ["Blocker", "AUTHENTIC PRODUCT EVIDENCE REQUIRED"]]} /></section>
  </Panel>;
}
