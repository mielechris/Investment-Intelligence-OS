...
function TuesdayCommandCenter({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  return <div className="mew-grid mew-tuesday-grid">
    <Panel id="tuesday-command" eyebrow="OPENING-DAY CONTROL" title="Tuesday Test Day Command Center" state="NOT_ACTIVATED" className="mew-wide">
      <section className="mew-tuesday-hero" aria-labelledby="mew-tuesday-hero-title"><span>TUESDAY PAPER TEST · NO REAL MONEY</span><h2 id="mew-tuesday-hero-title">10 pilots · 3 observations each · 30-credit maximum</h2><strong>Controller disabled — awaiting authorization</strong><div className="mew-rehearsal-result"><div><span>MONDAY REHEARSAL · SEPTEMBER 7, 2026</span><strong>CLOSED HOLIDAY · PASSED</strong><small>Provider requests 0 · Credits 0 · Candidates 0 · Operational activity 0</small></div><div><span>TUESDAY CONTROLLER</span><strong>INSTALLED NO · ACTIVATED NO</strong><small>Current phase: TUESDAY_PREMARKET_LOCKED · awaiting authorization</small></div></div><div className="mew-observation-phases"><article><b>1</b><span>Opening Evidence</span><small>Not scheduled until authorization</small></article><article><b>2</b><span>Intraday Mark</span><small>One bounded observation</small></article><article><b>3</b><span>Closing Mark</span><small>Reconcile without promotion</small></article></div><dl><div><dt>Ten instruments</dt><dd>Three calls maximum each</dd></div><div><dt>Global credit ceiling</dt><dd>30</dd></div><div><dt>Requests / credits today</dt><dd>0 / 0</dd></div><div><dt>Auto-reload</dt><dd>Disabled</dd></div><div><dt>Next allowable action</dt><dd>Authorize the governed Tuesday controller</dd></div><div><dt>Market / session</dt><dd>{scalar(market.state)}</dd></div><div><dt>Current evidence</dt><dd>Waiting for Tuesday</dd></div><div><dt>Candidate / Committee / Risk</dt><dd>No authenticated candidate</dd></div><div><dt>Corporate-action suspensions</dt><dd>None reported</dd></div><div><dt>Synthetic / post-close state</dt><dd>Not started / not ready</dd></div><div><dt>Browser invocation</dt><dd>Prohibited</dd></div><div><dt>Automatic retry</dt><dd>Prohibited</dd></div><div><dt>Operational trading</dt><dd>Disabled</dd></div></dl><div className="mew-governance-flow" aria-label="Governance flow">{["Evidence","Human Review","Committee","Risk","Synthetic Observation","Intraday Mark","Closing Mark","Post-Close Audit"].map((stage,index)=><span key={stage}>{stage}{index<7?<i aria-hidden="true">→</i>:null}</span>)}</div><aside className="mew-fail-closed"><strong>THE CONTROLLER STOPS BEFORE CREDENTIAL ACCESS WHEN:</strong><ul>{["Wrong session","Stale or future evidence","Wrong instrument","Missing candidate lineage","Human decision rejected","Committee rejection","Risk rejection","Budget exhausted","Corporate action unresolved","Unsafe authority"].map(reason=><li key={reason}>{reason}</li>)}</ul></aside><details><summary>Exact contract details</summary><p>CLOSED_HOLIDAY · TUESDAY_PREMARKET_LOCKED · WAITING_FOR_CURRENT_TUESDAY_EVIDENCE · FORWARD_ONLY_UNADJUSTED_OBSERVATION · CONTROLLER_DISABLED · AUTOMATIC_RETRY_PROHIBITED</p></details></section>
      <section className="mew-controller-status" aria-label="Tuesday commissioning status">
        <article><h3>Monday rehearsal</h3><dl><div><dt>Date</dt><dd>September 7, 2026</dd></div><div><dt>Session</dt><dd>Closed holiday · Passed</dd></div><div><dt>Requests</dt><dd>0</dd></div><div><dt>Credits</dt><dd>0</dd></div><div><dt>Candidates</dt><dd>0</dd></div><div><dt>Operational activity</dt><dd>0</dd></div></dl></article>
        <article><h3>Tuesday controller</h3><dl><div><dt>Installed</dt><dd>No</dd></div><div><dt>Activated</dt><dd>No</dd></div><div><dt>Current phase</dt><dd>Tuesday premarket locked</dd></div><div><dt>Human gate</dt><dd>Awaiting authorization</dd></div></dl></article>
      </section>
      <p className="mew-notice"><strong>Current human gate:</strong> awaiting authorization.</p>
...
function ProductAccountDirectory({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  useEffect(() => {
    const restore = () => { const value = window.location.hash.replace(/^#product-/, ""); const pilot = TUESDAY_PILOTS.find((item) => item.ticker.toLowerCase() === value.toLowerCase()); const product = PRODUCT_DESKS.find(([, name]) => name.toLowerCase().replaceAll(/[^a-z0-9]+/g, "-") === value.toLowerCase()); if (pilot) setSelected(pilot.ticker); else if (product) setSelected(product[1]); };
    restore(); window.addEventListener("hashchange", restore); return () => window.removeEventListener("hashchange", restore);
  }, []);
  const selectProduct = (value: string, hash: string) => { setSelected(value); window.history.pushState(null, "", `#product-${hash}`); window.dispatchEvent(new HashChangeEvent("hashchange")); requestAnimationFrame(() => { detailRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }); detailRef.current?.focus({ preventScroll: true }); }); };
  return <Panel id="product-account-directory" eyebrow="24 SEPARATE RESEARCH ACCOUNTS" title="Product Account Directory" state="AVAILABLE_EMPTY" className="mew-wide">
    <p>Twenty-four directly visible product accounts are grouped into ten Tuesday pilots and fourteen future source waves.</p>
    ...
    <header className="mew-directory-heading"><span>OPENING-DAY PRIORITY</span><h3>10 Tuesday Pilot Instruments</h3><p>Delayed data · forward-only paper research · no current position or recommendation</p></header>
    <div className="mew-pilot-directory" aria-label="Exactly ten Tuesday pilot instruments">{TUESDAY_PILOTS.map((pilot, index) => <button type="button" key={pilot.ticker} aria-pressed={selected === pilot.ticker} onClick={() => selectProduct(pilot.ticker, pilot.ticker.toLowerCase())}><span className="mew-pilot-number">PILOT {index + 1}</span><b>{pilot.ticker}</b><strong>{pilot.title}</strong><span>Independent synthetic basis: $10,000</span><em>Delayed data</em><span>Waiting for current Tuesday evidence</span><span>Forward-only paper research</span><small>Positions / trades: 0 / 0</small><small>Next observation: Opening Evidence</small><small>Eligibility: Not yet eligible</small><small>Blocker: {pilot.blocker}</small></button>)}</div>
    ...
    <header className="mew-directory-heading mew-future-heading"><span>FUTURE SOURCE WAVES</span><h3>14 Rooms — Not Available for Tuesday</h3><p>No activated source · no Tuesday observation scheduled · no research or paper eligibility</p></header>
    <div className="mew-future-directory" aria-label="Exactly fourteen future source wave rooms">{futureProducts.map(([department, name, classification], index) => { const hash=name.toLowerCase().replaceAll(/[^a-z0-9]+/g,"-"); return <button type="button" key={name} aria-pressed={selected === name} onClick={() => selectProduct(name, hash)}><b>{String(index + 1).padStart(2, "0")} · {name}</b><span>Independent measuring basis: $10,000</span><small>{department} · {classification}</small><em>Source not connected</em><small>No Tuesday observation scheduled</small><small>Eligibility: false · AUTHENTIC PRODUCT EVIDENCE REQUIRED</small></button>; })}</div>
  </Panel>;
}
