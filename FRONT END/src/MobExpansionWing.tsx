...
function TuesdayCommandCenter({ snapshot }: { snapshot: ExpansionSnapshot | null }) {
  ...
  const executor = record(section(snapshot, "operational_market_executor")?.data);
  ...
  return <div className="mew-grid mew-tuesday-grid" data-controller-version={controllerV2 ? "v2" : "v1"}>
    <Panel id="tuesday-command" eyebrow="OPENING-DAY CONTROL" title="Tuesday Test Day Command Center" state="NOT_ACTIVATED" className="mew-wide">
      ...
      <section className="mew-provider-readiness" aria-label="Operational market evidence executor">
        <header><span>AUTHENTIC EXECUTION GENERATION</span><h3>Operational Market Evidence Executor</h3><Status state={text(section(snapshot,"operational_market_executor")?.state,"UNAVAILABLE") as TruthState}/></header>
        <MetricGrid values={[["Installation",executor.installed],["Classification",executor.classification],["Phase",executor.phase],["Next gate",executor.next_gate],["Planned",executor.planned],["Dispatched",executor.dispatched],["Completed",executor.completed],["Remaining",executor.remaining],["Adopted canary",executor.adopted_canary_count],["Failed",executor.failed],["Ambiguous",executor.ambiguous],["Evidence receipts",executor.completed],["Confirmed credits",executor.confirmed_credits],["Ambiguous credits",executor.ambiguous_credits],["Released credits",executor.released_credits],["Credential accesses",executor.keychain_accesses],["Stage A",executor.stage_a],["Stage B",executor.stage_b],["Stage C",executor.stage_c]]}/>
        <p>Only authenticated receipt metadata is projected. Provider bodies, request identities, and credential material never enter the browser.</p>
      </section>
...
export { NorthstarProducts as NorthstarExpansionWing } from './NorthstarDestinations';
