...
test("schema version and server-issued provenance independently control presentation", () => {
  for (const phrase of ["controller_status_provenance", "AUTHENTIC_OPERATIONAL_STATE", "AUTHENTIC CONTROLLER STATUS",
    ...
    "iios-tuesday-controller-state-v2", "SIMULATED / REHEARSED — NOT OPERATIONAL", "Released credits", "Provenance", "Authority locked"])
  ...
  assert.match(source, /controllerProvenanceAvailable \? controllerProvenance : "UNAVAILABLE"/);
  assert.doesNotMatch(source, /controllerV2[^\n]+AUTHENTIC_OPERATIONAL_STATE/);
});
