...
export default function JesseIntelligencePanel() {
  ...
  return <section style={{maxWidth:"1110px",margin:"0 auto 28px",padding:"22px",border:"1px solid #4e4731",borderRadius:"14px",background:"rgba(8,12,17,.96)",color:"#eef2f5"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(260px,1fr))",gap:"12px",marginTop:"18px"}}>
      ...
      <div style={card}><b>Production input health</b><div>Research discoveries: {data.a?.institutional_discovery?.discovery_count ?? 0}</div><div>Production universe: {data.a?.strict_universe_verified ? "VERIFIED" : "NOT READY"} · {data.a?.active_production_index_universe?.symbol_count ?? 0} symbols</div><div>Universe source: {(data.a?.active_production_index_universe?.source_lineage || []).map((x:any)=>`${x.index}:${x.verified_complete ? "OK" : "FAIL"}`).join(" · ") || "—"}</div><div>Using prior verified snapshot: {data.a?.using_prior_verified_universe ? "YES" : "NO"}</div><div>CME FedWatch: {data.a?.cme_fedwatch?.configured ? `${data.a?.cme_fedwatch?.mode || "EOD"} CONFIGURED` : "NOT CONFIGURED"}</div><div>Fed snapshot verified: {data.a?.cme_fedwatch?.latest_snapshot_source_verified ? "YES" : "NO"}</div><div>Fail closed: {data.a?.production_inputs_fail_closed ? "YES" : "—"}</div></div>
