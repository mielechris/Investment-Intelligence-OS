...
import "./HistoricalReplayV721.css";
...
type ReplayPhase = "idle" | "moving" | "assembled";

type ValidationLayer = { availability?: string; age_seconds?: number | null; payload?: JsonObject | null };
type LivingOverview = {
  validation?: { layers?: { factory_telemetry?: ValidationLayer } };
  safety?: { backend_write_permission?: boolean; trade_execution_permission?: boolean; live_execution?: boolean };
};
type FactoryEvent = { event_type?: string | null; case_id?: string | null; entity_id?: string | null; created_at?: string | null; payload?: JsonObject | null };
type Promotion = {
  case_id?: string | null;
  ticker?: string | null;
...
const STATION_ROOM: Record<StationKey, RoomKey> = {
  radar: "pit", research: "war", agents: "bullpen", committee: "commission",
  risk: "risk", paper: "paper", monitoring: "monitoring", learning: "learning",
};
const HOME_ROOM: Record<LivingCastKey, RoomKey> = {
  max: "max", policy: "war", macro: "war", fundamentals: "bullpen",
  market_structure: "pit", commodities: "pit", geo_weather: "war", skeptic: "commission", portfolio: "risk",
};
...
function timeLabel(value: unknown): string {
  const parsed = parseTime(value);
  return parsed === null ? "TIME UNKNOWN" : new Date(parsed).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
}
...
function promotionFor(event: FactoryEvent | null, promotions: Promotion[]): Promotion | null {
  ...
  return caseId ? promotions.find((item) => text(item.case_id, "") === caseId) ?? null : null;
}
...
function formationOffset(index: number, count: number): string {
  const layouts: Record<number, number[]> = {
    1: [0], 2: [-1.7, 1.7], 3: [-2.6, 0, 2.6],
    4: [-3.3, -1.1, 1.1, 3.3], 5: [-4.1, -2.05, 0, 2.05, 4.1],
  };
  const values = layouts[Math.min(Math.max(count, 1), 5)] ?? layouts[5];
...
async function loadOverview(signal: AbortSignal): Promise<LivingOverview> {
  ...
  if (!response.ok) throw new Error(`V7.2.1 replay source unavailable: HTTP ${response.status}`);
  return response.json() as Promise<LivingOverview>;
}

export default function HistoricalReplayV721({ view }: { view: View }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        setError(reason instanceof Error ? reason.message : "V7.2.1 replay source unavailable");
      }
    };
    void refresh();
  ...
  const model = useMemo(() => {
    ...
    return { events, safeIndex, event, eventType, room, promotion, ticker, cast, context };
  ...
  const runReplay = () => {
    ...
    window.requestAnimationFrame(() => {
      ...
      setPhase("moving");
      arrivalTimer.current = window.setTimeout(() => setPhase("assembled"), 4850);
    });
  };
  const previous = () => {
  ...
  const targetRoom: RoomKey = model.room ?? "max";
  const active = phase !== "idle";
  const buttonCopy = phase === "moving" ? "THE CREW'S MOVIN'..." : phase === "assembled" ? "↻ RUN IT AGAIN" : "▶ RUN THE FUCKIN' REHEARSAL";

  return (
    <section className={`hr72-shell hr721-shell hr72-shell--${view} is-${phase}`} aria-label="V7.2.1 historical character replay theater">
      <header className="hr72-header">
        <div>
          <span>V7.2.1 · THE BACK ROOM SCREENING THEATER</span>
          <h2>REAL RECEIPTS. FAKE REENACTMENT. FILTHY MOUTHS.</h2>
          <p>Pick a real persisted IIOS event and rehearse the room choreography. The crew now stays put after arrival instead of snapping home like a cheap fuckin' PowerPoint.</p>
        </div>
        <div className="hr72-stamp"><strong>HISTORICAL REPLAY</strong><span>NOT LIVE · NOT MODEL OUTPUT</span></div>
      </header>

      <div className="hr72-truth">
        <span>EVENT · REAL PERSISTED 9G RECORD</span><span>REENACTMENT · PRESENTATION ONLY</span>
      ...
      <div className="hr72-controls">
        ...
        <div><span>RECEIPT {model.events.length ? model.safeIndex + 1 : 0} OF {model.events.length}</span><strong>{model.event ? readable(model.eventType, "UNKNOWN EVENT") : "NO PERSISTED EVENTS"}</strong><small>{model.event ? `${model.ticker} · ${timeLabel(model.event.created_at)} · ${ageLabel(model.event.created_at)}` : "WAITING FOR 9G"}</small></div>
        <button type="button" onClick={runReplay} disabled={!model.event}>{buttonCopy}</button>
        <button type="button" onClick={next} disabled={!model.events.length}>NEWER RECEIPT →</button>
      </div>

      {model.event ? <>
        <div className="hr72-warning"><strong>REHEARSAL / HISTORICAL REPLAY</strong><span>{mobReplayBannerLine(model.ticker, model.eventType)}</span></div>
        <div className="hr72-building" key={runId}>
          <div className="hr72-rooms">{ROOMS.map((room) => <article key={room.key} className={room.key === targetRoom ? "is-target" : ""}><span>{room.code}</span><strong>{room.label}</strong>{room.key === targetRoom ? <em>{phase === "assembled" ? "CREW ASSEMBLED" : "SCENE ROOM"}</em> : null}</article>)}</div>
          <div className="hr72-corridor" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /><i /><i /></div>
          <div className="hr72-travelers">{model.cast.map((key, index) => {
            const style: TravelerStyle = {
              "--start-x": roomX(HOME_ROOM[key]), "--end-x": roomX(targetRoom),
              "--formation-offset": formationOffset(index, model.cast.length), "--travel-delay": `${index * 180}ms`,
            };
            return <div className={`hr72-traveler ${key === "skeptic" ? "is-red" : ""}`} key={`${runId}:${key}`} style={style}>
              <div className="hr72-traveler-avatar"><CinematicCharacterPortrait characterKey={key} active={active} reacting={phase === "moving"} variant={key === "max" ? "boss" : "card"} showLabel={false} /></div>
              <strong>{LIVING_CAST[key].displayName}</strong><span>{phase === "assembled" ? `AT ${ROOMS.find((room) => room.key === targetRoom)?.code}` : `${ROOMS.find((room) => room.key === HOME_ROOM[key])?.code} → ${ROOMS.find((room) => room.key === targetRoom)?.code}`}</span>
            </div>;
          })}</div>
        </div>

        <div className="hr72-scene">
          <aside><span>THE RECEIPT</span><strong>{readable(model.eventType, "UNKNOWN EVENT")}</strong><dl>
            <div><dt>TICKER</dt><dd>{model.ticker}</dd></div><div><dt>CASE</dt><dd>{text(model.event.case_id, "NO CASE")}</dd></div>
            <div><dt>ENTITY</dt><dd>{text(model.event.entity_id, "NO ENTITY")}</dd></div><div><dt>ROOM</dt><dd>{ROOMS.find((room) => room.key === targetRoom)?.label}</dd></div><div><dt>AGE</dt><dd>{ageLabel(model.event.created_at)}</dd></div>
          </dl></aside>
          <section className="hr72-dialogue"><header><span>THE FAMILY REENACTS THE RECEIPT</span><strong>{phase === "assembled" ? "CREW IN THE ROOM · MOB-NOIR REPLAY" : "MOB-NOIR NARRATIVE · NOT LITERAL AGENT SPEECH"}</strong></header><div>
            {model.cast.map((key) => <article key={`${model.eventType}:${key}`} className={key === "skeptic" ? "is-red" : ""}><div className="hr72-dialogue-avatar"><CinematicCharacterPortrait characterKey={key} active={active} reacting={phase === "moving"} variant={key === "max" ? "boss" : "scene"} showLabel={false} /></div><div><header><strong>{LIVING_CAST[key].displayName}</strong><span>{LIVING_CAST[key].title}</span></header><blockquote>“{mobReactionLine(key, model.context)}”</blockquote><small>PRESENTATION-ONLY CHARACTER VOICE · EVENT IDENTITY REMAINS PERSISTED</small></div></article>)}
          </div></section>
        </div>
