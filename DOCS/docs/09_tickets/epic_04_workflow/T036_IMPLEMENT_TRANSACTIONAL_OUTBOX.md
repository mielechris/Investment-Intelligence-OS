# T036 — Implement transactional outbox
...
Write domain events atomically with state changes.
...
- [ ] Outbox row commits with domain transaction
- [ ] Rollback removes both
- [ ] Unpublished events queryable
...
- [ ] Commit references `T036`.

## Suggested Commit

`feat(workflow): implement transactional outbox [T036]`
