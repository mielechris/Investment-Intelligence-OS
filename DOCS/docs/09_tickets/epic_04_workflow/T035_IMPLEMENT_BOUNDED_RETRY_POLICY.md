# T035 — Implement bounded retry policy
...
Retry only transient failures with backoff, jitter, and max attempts.
...
- [ ] Retryable error moves to retry wait
- [ ] Permanent error becomes terminal
...
- [ ] Commit references `T035`.

## Suggested Commit

`feat(workflow): implement bounded retry policy [T035]`
