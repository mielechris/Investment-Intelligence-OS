# SPEC-001 — Data Ingestion Engine
...
Retrieve, preserve, normalize, timestamp, deduplicate, version, and replay approved source data.
...
1. Source registry and connector adapters.
2. Immutable raw storage before interpretation.
3. Parser and normalizer separation.
4. Four time semantics where applicable.
5. Durable checkpoints.
6. Rate limits and bounded retries.
7. Deduplication and revisions.
8. Quarantine.
9. Source health.
10. Replay from raw records.
...
- `ParseResult`
- `NormalizationResult`
- `CanonicalEvent or canonical data object`
- `ConnectorCheckpoint`
...
- [ ] Duplicate fetch creates one canonical effect.
- [ ] Changed content creates revision.
- [ ] Checkpoint advances only after durable raw storage.
- [ ] Malformed or prohibited data quarantines.
- [ ] Raw records replay without refetching.

## 10. Required Tests

- Verify: Duplicate fetch creates one canonical effect.
- Verify: Changed content creates revision.
- Verify: Checkpoint advances only after durable raw storage.
- Verify: Malformed or prohibited data quarantines.
- Verify: Raw records replay without refetching.
