...
Expose typed, versioned application capabilities and vendor-neutral provider adapters.
...
1. Versioned HTTP JSON API.
2. OpenAPI contract.
3. Stable errors.
4. Durable jobs for long work.
...
- [ ] Long task returns durable job ID.
- [ ] Provider errors normalized.
- [ ] Paper mode cannot call live integration.

## 10. Required Tests

- Verify: OpenAPI generated.
- Verify: Unauthorized action rejected.
- Verify: Long task returns durable job ID.
- Verify: Provider errors normalized.
- Verify: Paper mode cannot call live integration.
