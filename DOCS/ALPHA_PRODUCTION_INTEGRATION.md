...
## Connected short-session offline implementation

`alpha_observation_execution.run_observation` now reconstructs an explicit
`iios-alpha-short-gateway-plan-v1` from all independently pinned planning inputs.
It invokes the public gateway `qualify` path for three ordered requests, including
both reservation layers, actual runtime-file admission, freshness/coverage
checks, exclusive receipt publication and independent final reconciliation.
The short cutoff is checked before both reservations and forwarded through the
transport to the existing `bounded_https.before_request` hook. Failure stops
subsequent requests; consumed/ambiguous budget remains consumed. Cleanup runs
independently of the primary outcome, and journal contents are rechecked after it.
Actual UTC and monotonic bounds both apply; there is no clock-budget reset.

This connector admits `OFFLINE_TEST` only. Integrated tests substitute external
credential/network/lifecycle effects while executing the real gateway and
filesystem journal path. They are not a production launch implementation. The
missing production effect owner must implement real scheduler/publisher/backend
roles and a separately qualified live capability. `OfflineLifecycle` is an explicit
test effect seam; `LaunchOwner` remains receipt-only and cannot be used to replace
it in live mode. This source gap remains named `SHORT_NATIVE_ADMISSION_PENDING`.
The old CLI, pending-native, shadow-service and confinement guards remain intact.

`alpha_denial_collector` implements one pinned `/usr/bin/log show` query for at
most a ten-second dummy-trial window, with a five-second capture deadline and
4,096-byte caps per stream. It uses the Apple-documented reporting subsystem and
violation category, fixed source filters, target PID and exact report-field
matching. Target process identity must match before and after collection. No raw
log text, arbitrary path, error message or raw-event hash is retained. Overflow,
missing/redacted fields, emitter/format differences and capture failures remain
UNKNOWN/unverified. It never signals an unverified collector or configures logs.
`OS_DENIAL_REPORT_MATCH` is correlated evidence, not full confinement acceptance:
a controlled allowed/denied comparison and independently reviewed launch/profile
binding are still necessary. App Sandbox documentation does not prove equivalent
report delivery for this host's deprecated sandbox-exec. Native availability has
not been tested and must not be asserted absent or proven.

`alpha_runtime_bootstrap` binds a non-circular recipe: verified OS tools extract
and relocate a vendor interpreter into a separate bootstrap destination; static
closure and final-bootstrap-destination qualification precede any Python assembly
script. That interpreter then runs the existing assembler to a different final
runtime destination. Both destinations need separate provenance/import/linkage
acceptance. No production runtime is needed to build its own bootstrap. Existing
Apple CLT Python was evaluated as a simpler alternative; its trust verification
was unsuccessful in the inspected context, so it was not admitted or executed.
The vendor-bootstrap recipe is a design contract, not an archive extractor or an
execution approval; actual archive layout and transformation commands must be
reviewed from acquired, signed bytes before execution. No installed framework is
copied, chmodded or relabeled to satisfy production ownership checks.

Sources for the diagnostic/build design are Apple's “Discovering and diagnosing
App Sandbox violations”, the selected host's `log(1)` and `sandbox-exec(1)` manuals,
and CPython's official macOS/build documentation. Tool and report provenance must
be verified on the selected host. Existing account-use/selector declarations and
the accepted synthetic milestone are preserved. No current account evidence,
production authority or real provider qualification is generated by these tests.

## Truth Spine three-request observation integration

The observation path is a separate, explicitly selected contract. It reuses
`OwnedChildren`, the full-day service's scheduler/publisher/backend roles, leases,
startup identity inspection and the existing provider gateway. Shadow `run`,
`load_config`, `SessionSupervisor`, `GenerationStore` and DENY-only authority
semantics remain unchanged. The component-only child and launch harness remain
component-only. No second factory, service installation or automatic activation
is introduced.

`admit_observation_execution` reconstructs the short candidate and independently
pinned immutable runtime, release, account documents, selected-host qualification,
confinement qualification, lifecycle qualification and dated execution grant.
An owner must independently review those qualification pins; a self-consistent
hash or a component/synthetic PASS is not qualification. The release verifier
checks its exact source inventory and static import graph. Runtime admission also
checks the actual interpreter, imported module origins, complete immutable files,
TLS and numeric provider address. Release copies must match the pinned runtime's
source copies exactly. Final-location native dependency qualification remains a
separate prerequisite.

The parent uses `run_admitted_observation` with the existing concrete MacKeychain
and NativeHTTPS boundaries. The offline entrypoint still rejects live manifests.
Exactly three separately reserved requests retain their original time windows,
maximum costs, no-retry rules and ephemeral-data controls. The parent produces
an atomic exclusive dispatcher checkpoint only after verifying each returned
receipt and both reservation/completion chains. Readers never treat an
in-progress journal write as successful upstream evidence.

The scheduler attests the admitted journal state; the publisher derives bounded
coverage from verified receipts. `ObservationProbeReader` separately verifies
owned processes, startup receipts, fresh heartbeats, trusted dispatcher response
pins and the published projection. GET/HEAD only read evidence. They cannot
capture, dispatch, repair, renew or refresh timestamps. The projection states
TEN_SYMBOL_PILOT_ONLY, records the evidence mode, and explicitly states that
cases, agents, Committee and Risk have not run. Three successful requests do not
establish full-universe, full-day or continuous realtime market truth.

Startup uses one descriptor-bound monotonic-nanosecond deadline matching the UTC
short plan. The fixed confined child commands receive a minimal environment and
bounded pipes, never provider credentials. The existing owner requires three
complete OS observations and the startup receipt before ACK; backend ACK also
requires exact listener ownership. Pinned loopback TLS precedes gateway dispatch.
Every created child is retained before stdio setup can fail. Cooperative stop
receipts and independently verified exits are required for successful shutdown;
forced termination or empty ports alone cannot satisfy that result. Each fallback
signal requires ownership re-verification and remaining cleanup time. Cleanup
continues independently across roles, with primary and cleanup failures retained.

Preparation CI remains offline-only. Its audit guard precedes test imports,
rejects process/network/credential effects, confines disposable writes and fails
on attempted forbidden effects even when a test catches them. Explicitly selected
compatibility tests preserve native tests for separate qualification. Exported
results bind the exact commit, tree, source/test/workflow inventories, environment
and loaded dependency files. Hosted success never qualifies the selected Mac.

Before any native qualification, separately approve and pin: the acquired runtime
and final-location closure; exact release and control files; selected Mac OS and
architecture; fixed sandbox launcher/profile; disposable loopback TLS materials;
all tool hashes; exclusive absent output root; port; UTC/monotonic startup and
cleanup deadlines; role-specific filesystem/network/subprocess rules; and the
ownership/cleanup procedure. Use dummy credentials and disposable denied targets
only. Controlled comparisons and attributable OS denial evidence must pass;
EACCES/EPERM alone remains unattributed. Test normal and failed startup, altered
parents, listener/TLS mismatch, PID reuse, partial startup, cooperative timeout,
independent cleanup and stable clearance. Preserve every failed attempt; no
native retry or real-provider observation is authorized by this document.

Only after native component/lifecycle/confinement acceptance and current reviewed
endpoint, realtime feed, coverage, rights, retention, selector, rate, billing and
cost evidence may a separate three-request owner grant be prepared. It must bind
the accepted source and all actual artifacts, request/cost caps, session date,
stop conditions and cleanup. Broker connection, paper-order permission,
trade-execution permission and live execution remain false in all scopes.
